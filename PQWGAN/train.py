...
if __name__ == "__main__":
    # Hardcoded values
    classes = "01"   # string of class digits, e.g. "36"
    dataset = "mnist"  # "mnist" or "fmnist"
    patches = 7  # number of sub-generators [1, 2, 4, 7, 14, 28]
    layers = 8  # layers per sub-generator
    qubits = 7  # number of data qubits per sub-generator
    batch_size = 25
    out_folder = "outputs"
    checkpoint = 0
    randn = False  # use torch.randn if True, torch.rand if False
    patch_shape = (28, 4)  # shape of each patch
    qcritic = False  # whether to use quantum critic

    train(classes, dataset, patches, layers, qubits, batch_size, out_folder, checkpoint, randn, patch_shape, qcritic)
